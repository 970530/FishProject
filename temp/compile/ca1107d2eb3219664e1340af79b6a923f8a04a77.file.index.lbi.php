<?php /* Smarty version Smarty-3.1.7, created on 2017-04-15 10:29:21
         compiled from "C:/xampp/htdocs/template\bty2\index\index.lbi" */ ?>
<?php /*%%SmartyHeaderCode:1814158f18581c404e0-48709918%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'ca1107d2eb3219664e1340af79b6a923f8a04a77' => 
    array (
      0 => 'C:/xampp/htdocs/template\\bty2\\index\\index.lbi',
      1 => 1492222643,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '1814158f18581c404e0-48709918',
  'function' => 
  array (
  ),
  'variables' => 
  array (
    'i' => 0,
    'recommend' => 0,
    'video_recommmend' => 0,
    '_lang' => 0,
    'v' => 0,
    'chufang' => 0,
    'keting' => 0,
    'woshi' => 0,
    'shufang' => 0,
    'yimaojian' => 0,
    'new_join' => 0,
    'article' => 0,
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.7',
  'unifunc' => 'content_58f18581cdc90',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_58f18581cdc90')) {function content_58f18581cdc90($_smarty_tpl) {?><style>
	body{
		background-color: #efefef;
	}
	.inline.hidden-xs {
  		display: inline !important;
  	}		
	.chosen_wrap{
	}
	.img_list{
		margin-bottom: 10px;
		cursor: pointer;
	}
	.title_cover{
		height: 30px;
		background-color: rgba(138,137,137,0.5);
		position: absolute;
		bottom: 0px;
		width: 100%;
		color: #fff;
		line-height: 30px;
		font-size: 14px;
		padding-left: 10px;
	}
	.more{
		margin-top: 10px;
		text-align: right;
		display: block;
	}
	.of_hide{
		display: block;
	    overflow: hidden;
	    text-overflow: ellipsis;
	}
	.padding0{
		padding: 0;
	}
	.cards .card .card-content span{
		width: 100%;
		height: 20px;
	}
	.content_nav{
		/*margin-top: 18px;*/
	}
	.content_nav .col-md-2 div{
		height: 60px;
		line-height: 60px;
		text-align: center;
		font-size: 23px;
		border-radius: 8px;
		cursor: pointer;
		color:#fff;
	}
	.content_nav .col-xs-4{
		margin-top: 10px;
	}
	.kf_block{
		background-color: #EA644A;
	}
	.gj_block{
		background-color: #F1A325;
	}
	.ly_block{
		background-color: #38B03F;
	}
	.jz_block{
		background-color: #03B8CF;
	}
	.jy_block{
		background-color: #BD7B46;
	}
	.xw_block{
		background-color: #8666B8;
	}
	h2 { margin:5px;}
</style>
<script>
	$(function(){
		$("#top_frame").load(function() {
			console.log($(this).contents().find("body").height());
			$(this).height($(window).height()*0.7);
		})
	})
</script>
<script src="jquery.js"></script>
<div style="width:100%; ">
<!-- TOP轮播图开始 -->
<div class="banner_box">
			<div class="banner">
				<img src="/static/images/indexbanner01.png">
				<img src="/static/images/indexbanner02.png">
				<img src="/static/images/indexbanner03.png">
				<img src="/static/images/indexbanner01.png">
			</div>
			<ul class="banner_dian">
				<li class="currentLi"></li>
				<li></li>
				<li></li>
			</ul>
			<a class="btn_left" href="javascript:void(0)"><img src="/static/images/jt01.png" /></a>
			<a class="btn_right" href="javascript:void(0)"><img src="/static/images/jt02.png" /></a>
		</div>
<script src="/static/js/LUNBO.JS"></script>
<!-- TOP轮播图结束 -->
</div>
<div class="container" style="width:1260px">
	<div class="row" style="margin-top:10PX; margin-bottom:10PX;">
		<div class="col-md-3">
			<h2 class="text-muted">精选全景<small style="margin-left:10px;">优质的全景作品赏析</small></h2>
		</div>
	</div>
	<div class="row chosen_wrap" >
		<div class="col-md-5" style="height:445px;">
			<div id="myNiceCarousel" class="carousel slide" data-ride="carousel">
			  <!-- 轮播图开始 -->
			  <div class="carousel-inner">
			   <?php  $_smarty_tpl->tpl_vars['i'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['i']->value = 1;
  if ($_smarty_tpl->tpl_vars['i']->value<4){ for ($_foo=true;$_smarty_tpl->tpl_vars['i']->value<4; $_smarty_tpl->tpl_vars['i']->value++){
?>
			    <div class="item <?php if ($_smarty_tpl->tpl_vars['i']->value==1){?>active<?php }?>">
			     <img src="<?php echo $_smarty_tpl->tpl_vars['recommend']->value[$_smarty_tpl->tpl_vars['i']->value]['thumb_path'];?>
"  class="img-responsive" style="width: 100%;height: 445px;">
			      <div class="carousel-caption">
			        <h3><?php echo $_smarty_tpl->tpl_vars['recommend']->value[$_smarty_tpl->tpl_vars['i']->value]['name'];?>
</h3>
			      </div>
			    </div>
			   <?php }} ?>
			  </div>
              <!-- 轮播图结束 -->
			  <!-- 项目切换按钮 -->
			   <a class="left carousel-control" href="#myNiceCarousel" data-slide="prev" style="z-index:9">
			     <span class="icon icon-chevron-left"></span>
			   </a>
			   <a class="right carousel-control" href="#myNiceCarousel" data-slide="next">
			     <span class="icon icon-chevron-right"></span>
			   </a>
			 </div>
		</div>
		<div class="col-md-7">
			<div class="row chosen_wrap">
			    <?php  $_smarty_tpl->tpl_vars['i'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['i']->value = 4;
  if ($_smarty_tpl->tpl_vars['i']->value<10){ for ($_foo=true;$_smarty_tpl->tpl_vars['i']->value<10; $_smarty_tpl->tpl_vars['i']->value++){
?>
				<div class="col-md-4 col-sm-4 col-xs-6 img_list">
					<a href="/tour/<?php echo $_smarty_tpl->tpl_vars['recommend']->value[$_smarty_tpl->tpl_vars['i']->value]['view_uuid'];?>
" target="_blank">
						<img src="<?php echo $_smarty_tpl->tpl_vars['recommend']->value[$_smarty_tpl->tpl_vars['i']->value]['thumb_path'];?>
" class="img-responsive" style="width:222px; height:217px;">
					</a>
					<div style="position:relative;">
						<div class="title_cover of_hide"><?php echo $_smarty_tpl->tpl_vars['recommend']->value[$_smarty_tpl->tpl_vars['i']->value]['name'];?>
</div>
					</div>
				</div>
			    <?php }} ?>    
			</div>
		</div>
	</div>
    <div class="row" style="margin-top:10PX;">
		<div class="col-xs-12">
			<h2 class="text-muted">推荐视频<small style="margin-left:10px;">为您推荐的优质视频作品</small><a href="/videos"><small class="text-muted  pull-right more">更多>></small></a></h2>
		</div>
	</div>
    <div class="row" id="new_join">
		<div class="cards" style="margin:0; height:260px; overflow:hidden;">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['video_recommmend']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-4 col-sm-4 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="<?php echo $_smarty_tpl->tpl_vars['_lang']->value['cdn_host'];?>
video/play.html?vid=<?php echo $_smarty_tpl->tpl_vars['v']->value['id'];?>
">
                   <div style="width:300px; height:190px; overflow:hidden; margin:0 auto;">
                     <div style="position:absolute; z-index:0;width:300px;; height:190px;overflow:hidden;">
                       <img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="300px" height="190px">
                     </div>
                     <div style="position:absolute; z-index:2;width:80px; height:80px; left:105px;; top:60px;">
                       <img src="/template/<?php echo $_smarty_tpl->tpl_vars['_lang']->value['moban'];?>
/images/icon/x-1.png">
                     </div>
                     <div style="clear:both"></div>
                   </div>
                 </a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0" style="width:100%; text-align:center;">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['vname'];?>
</strong>
			     	</div>
			     </div>
			   </div>
			 </div>
			<?php } ?>
			<!--卡片列表循环结束-->
		</div>
	</div>
    <div class="recommend">
    <ul class="recommendnav">
   		<li class="recommendtitle">定制设计推荐</li> 	
        <li class="gustroom">客厅</li>
        <li class="bedroom">卧室</li>
        <li class="bookroom">书房</li>
        <li class="clothroom">衣帽间</li>
        <li class="cook">厨房</li>
    </ul>
    <div class="recommend1">
    	<!--一个卡片列表行-->
	<div class="row cook1">
		<div class="cards" style="margin:0;height:730px; overflow:hidden">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['chufang']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-3 col-sm-3 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="/tour/<?php echo $_smarty_tpl->tpl_vars['v']->value['view_uuid'];?>
"><div style="width:270px; height:260px; overflow:hidden; margin-top:15px;margin-left:15px;margin-right:15px;"><img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="100%"></div></a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
</strong> 
			     	</div>
			     	<div class="col-md-3 col-xs-4 of_hide padding0" style="background:url(/static/images/skin1/vr-btn-good-click.png) no-repeat;background-size:30%;">
			     		<div class="text-danger" style=" margin-left:10px;"> <?php echo $_smarty_tpl->tpl_vars['v']->value['praised_num'];?>
</div>
			     	</div>
			     </div>
				 <div class="card-content text-muted">
			       <span class="of_hide"><?php echo mb_substr($_smarty_tpl->tpl_vars['v']->value['profile'],0,15);?>
……</span>
			     </div>
			   </div>
			 </div>
			<?php } ?>	         
			<!--卡片列表循环结束-->	
		</div>
	</div>
	<!--一个卡片列表行结束-->
    	<!--一个卡片列表行-->
	<div class="row gustroom1">
		<div class="cards" style="margin:0;height:730px; overflow:hidden">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['keting']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-3 col-sm-3 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="/tour/<?php echo $_smarty_tpl->tpl_vars['v']->value['view_uuid'];?>
"><div style="width:270px; height:260px; overflow:hidden; margin-top:15px;margin-left:15px;margin-right:15px;"><img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="100%"></div></a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
</strong> 
			     	</div>
			     	<div class="col-md-3 col-xs-4 of_hide padding0" style="background:url(/static/images/skin1/vr-btn-good-click.png) no-repeat;background-size:30%;">
			     		<div class="text-danger" style=" margin-left:10px;"> <?php echo $_smarty_tpl->tpl_vars['v']->value['praised_num'];?>
</div>
			     	</div>
			     </div>
				 <div class="card-content text-muted">
			       <span class="of_hide"><?php echo mb_substr($_smarty_tpl->tpl_vars['v']->value['profile'],0,15);?>
……</span>
			     </div>
			   </div>
			 </div>
			<?php } ?>  
			<!--卡片列表循环结束-->	
		</div>
	</div>
	<!--一个卡片列表行结束-->
<!--一个卡片列表行-->
	<div class="row bedroom1">
		<div class="cards" style="margin:0;height:730px; overflow:hidden">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['woshi']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-3 col-sm-3 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="/tour/<?php echo $_smarty_tpl->tpl_vars['v']->value['view_uuid'];?>
"><div style="width:270px; height:260px; overflow:hidden; margin-top:15px;margin-left:15px;margin-right:15px;"><img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="100%"></div></a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
</strong> 
			     	</div>
			     	<div class="col-md-3 col-xs-4 of_hide padding0" style="background:url(/static/images/skin1/vr-btn-good-click.png) no-repeat;background-size:30%;">
			     		<div class="text-danger" style=" margin-left:10px;"> <?php echo $_smarty_tpl->tpl_vars['v']->value['praised_num'];?>
</div>
			     	</div>
			     </div>
				 <div class="card-content text-muted">
			       <span class="of_hide"><?php echo mb_substr($_smarty_tpl->tpl_vars['v']->value['profile'],0,15);?>
……</span>
			     </div>
			   </div>
			 </div>
			<?php } ?>     
			<!--卡片列表循环结束-->	
		</div>
	</div>
	<!--一个卡片列表行结束-->
    <!--一个卡片列表行-->
	<div class="row bookroom1">
		<div class="cards" style="margin:0;height:730px; overflow:hidden">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['shufang']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-3 col-sm-3 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="/tour/<?php echo $_smarty_tpl->tpl_vars['v']->value['view_uuid'];?>
"><div style="width:270px; height:260px; overflow:hidden; margin-top:15px;margin-left:15px;margin-right:15px;"><img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="100%"></div></a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
</strong> 
			     	</div>
			     	<div class="col-md-3 col-xs-4 of_hide padding0" style="background:url(/static/images/skin1/vr-btn-good-click.png) no-repeat;background-size:30%;">
			     		<div class="text-danger" style=" margin-left:10px;"> <?php echo $_smarty_tpl->tpl_vars['v']->value['praised_num'];?>
</div>
			     	</div>
			     </div>
				 <div class="card-content text-muted">
			       <span class="of_hide"><?php echo mb_substr($_smarty_tpl->tpl_vars['v']->value['profile'],0,15);?>
……</span>
			     </div>
			   </div>
			 </div>
			<?php } ?>     
			<!--卡片列表循环结束-->	
		</div>
	</div>
	<!--一个卡片列表行结束-->
<!--一个卡片列表行-->
	<div class="row clothroom1">
		<div class="cards" style="margin:0;height:730px; overflow:hidden">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['yimaojian']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-3 col-sm-3 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="/tour/<?php echo $_smarty_tpl->tpl_vars['v']->value['view_uuid'];?>
"><div style="width:270px; height:260px; overflow:hidden; margin-top:15px;margin-left:15px;margin-right:15px;"><img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="100%"></div></a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
</strong> 
			     	</div>
			     	<div class="col-md-3 col-xs-4 of_hide padding0" style="background:url(/static/images/skin1/vr-btn-good-click.png) no-repeat;background-size:30%;">
			     		<div class="text-danger" style=" margin-left:10px;"> <?php echo $_smarty_tpl->tpl_vars['v']->value['praised_num'];?>
</div>
			     	</div>
			     </div>
				 <div class="card-content text-muted">
			       <span class="of_hide"><?php echo mb_substr($_smarty_tpl->tpl_vars['v']->value['profile'],0,15);?>
……</span>
			     </div>
			   </div>
			 </div>
			<?php } ?>    
			<!--卡片列表循环结束-->	
		</div>
	</div>
	<!--一个卡片列表行结束-->
    <div class="clearfix"></div>
    </div>
</div>
<script>
	$(".recommend1 .gustroom1").show()
	$(".recommend1 .bedroom1,.clothroom1,.cook1,.bookroom1").hide()
	$(".recommendnav .cook").click(
		function(){
			$(".recommend1 .cook1").show()
			$(".recommend1 .bedroom1,.clothroom1,.gustroom1,.bookroom1").hide()
			}
	)
	$(".recommendnav .gustroom").click(
		function(){
			$(".recommend1 .gustroom1").show()
			$(".recommend1 .bedroom1,.clothroom1,.cook1,.bookroom1").hide()
			}
	)
	
	$(".recommendnav .bedroom").click(
		function(){
			$(".recommend1 .bedroom1").show()
			$(".recommend1 .cook1,.clothroom1,.gustroom1,.bookroom11").hide()
			}
	)
	$(".recommendnav .clothroom").click(
		function(){
			$(".recommend1 .clothroom1").show()
			$(".recommend1 .bedroom1,.cook1,.gustroom1,.bookroom1").hide()
			}
	)
	$(".recommendnav .bookroom").click(
		function(){
			$(".recommend1 .bookroom1").show()
			$(".recommend1 .bedroom1,.cook1,.gustroom1,.clothroom1").hide()
			}
	)
</script>
    <div class="row">
		<div class="col-xs-12">
			<h2 class="text-muted">最新上传<small style="margin-left:10px;">刚刚上传的最新全景巡游</small><a href="/pictures"><small class="text-muted  pull-right more">更多>></small></a></h2>
		</div>
	</div>
    <!--一个卡片列表行-->
	<div class="row" id="new_join">
		<div class="cards" style="margin:0; height:670px; overflow:hidden;">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['new_join']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
			<div class="col-md-3 col-sm-3 col-xs-6 col-lg-3">
			   <div class="card" href="###">
			     <a target="_blank" href="/tour/<?php echo $_smarty_tpl->tpl_vars['v']->value['view_uuid'];?>
"><div style="width:300px; height:275px; overflow:hidden; margin:0 auto;"><img src="<?php echo $_smarty_tpl->tpl_vars['v']->value['thumb_path'];?>
" alt="<?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
" width="100%"></div></a>
			     <div class="card-heading">
			     	<div class="col-md-9 col-xs-8 of_hide padding0">
			     		<strong class="text-primary"><?php echo $_smarty_tpl->tpl_vars['v']->value['name'];?>
</strong>
			     	</div>
			     	<div class="col-md-3 col-xs-4 of_hide padding0" style="background:url(/static/images/skin1/vr-btn-good-click.png) no-repeat;background-size:30%;">
			     		<div class="text-danger" style=" margin-left:10px;;"><?php echo $_smarty_tpl->tpl_vars['v']->value['praised_num'];?>
</div>
			     	</div>
			     </div>
			   </div>
			 </div>
			<?php } ?>
			<!--卡片列表循环结束-->
		</div>
	</div>
	<!--一个卡片列表行结束-->
    <div class="recommend">
    <ul class="recommendnav">
   		<li class="recommendtitle">学习交流</li> 	
        <li>经验分享与全景入门基础</li>
    </ul>
    <div class="recommend1" style="height:230px;">
    <!--一个卡片列表行-->
	<div class="row cook1">
		<div class="cards" style="margin:0;overflow:hidde; height:230px">
			<!--卡片列表循环-->
			<?php  $_smarty_tpl->tpl_vars['v'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['v']->_loop = false;
 $_smarty_tpl->tpl_vars['k'] = new Smarty_Variable;
 $_from = $_smarty_tpl->tpl_vars['article']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['v']->key => $_smarty_tpl->tpl_vars['v']->value){
$_smarty_tpl->tpl_vars['v']->_loop = true;
 $_smarty_tpl->tpl_vars['k']->value = $_smarty_tpl->tpl_vars['v']->key;
?>
                <div style="width:620px; border:1px solid #ccc; margin:0 10px 20px 10px; padding:10px; float:left">
  					<div style="height:40px; line-height:40px"><h2 style="margin:0px; font-size:24px;"><?php echo $_smarty_tpl->tpl_vars['v']->value['title'];?>
</h2></div>
 			        <div style="height:25px; line-height:25px"><span>关键词：<?php echo $_smarty_tpl->tpl_vars['v']->value['keywords'];?>
</span><span style="margin-left:20px;">发布时间：<?php echo $_smarty_tpl->tpl_vars['v']->value['createtime'];?>
</span></div>
				</div>		
			<?php } ?>	
			<!--卡片列表循环结束-->	
		</div>
	</div>
	<!--一个卡片列表行结束-->
		</div>
	</div>
	<!--一个卡片列表行结束-->
    <div class="clearfix"></div>
    </div>
</div>
</div><?php }} ?>